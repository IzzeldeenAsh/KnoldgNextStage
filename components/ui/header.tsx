...
async function fetchIndustriesFromAPI(locale: string): Promise<Industry[]> {
  try {
    const res = await fetch("https://api.foresightaa.co/api/platform/industries/menu", {
      method: "POST",
      headers: {
...
const { isLoading: isAppLoading, setIsLoading: setAppLoading } = useLoading();
  ...
  const switchLocale = (locale: string) => {
    ...
    if (isProduction) {
      cookieParts.push(`Domain=.foresightaa.co`); // leading dot = include subdomains
          ...
          <ul className="flex justify-end items-center flex-shrink-0">
            ...
            {isLoading ? (
              ...
              <li>
                ...
                href={`https://app.foresightaa.co/auth/login?returnUrl=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}`}>
