...
export default function AuthBanner({ onSignUp, onLogin }: AuthBannerProps) {
  ...
  const handleSignUp = () => {
    if (onSignUp) {
      ...
      window.location.href = `https://app.foresightaa.co/auth/sign-up`;
    }
  };
  ...
  const handleLogin = () => {
    if (onLogin) {
      ...
      window.location.href = `https://app.foresightaa.co/auth/login?redirect_uri=${encodeURIComponent(window.location.href)}`;
    }
  };
