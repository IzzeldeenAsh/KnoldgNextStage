      ...
      try {
        ...
        if (entityType === "insighter") {
          ...
          let response = await fetch(
            `https://api.foresightaa.co/api/platform/insighter/profile/${uuid}`,
            {
              headers: {
          ...
          if (response.ok) {
            ...
            response = await fetch(
              `https://api.foresightaa.co/api/platform/company/profile/${uuid}`,
              {
                headers: {
                  "Content-Type": "application/json",
                  Accept: "application/json",
      ...
      try {
        ...
        let url =
          entityType === "insighter"
            ? `https://api.foresightaa.co/api/platform/insighter/knowledge/${uuid}?page=${knowledgePage}&per_page=12`
            : `https://api.foresightaa.co/api/platform/company/knowledge/${uuid}?page=${knowledgePage}&per_page=12`;
      ...
      if (isAuth && token) {
        try {
          const response = await fetch(
            "https://api.foresightaa.co/api/account/profile",
            {
              headers: {
                "Content-Type": "application/json",
                Accept: "application/json",
  ...
  const fetchMeetingAvailability = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://api.foresightaa.co/api/account/meeting/available/hours/${uuid}`,
        {
