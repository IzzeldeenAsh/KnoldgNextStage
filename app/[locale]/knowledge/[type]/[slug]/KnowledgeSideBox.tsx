...
const KnowledgeSideBox = ({
  ...
  const handleReadLaterToggle = async () => {
    ...
    try {
      ...
      const url =  `https://api.foresightaa.co/api/account/favorite/knowledge/${knowledgeSlug}`
          ...
          {!isOwner && (
            <>
              {purchased_status === 'purchased' ? (
                <button 
                  onClick={() => window.location.href = (process.env.NEXT_PUBLIC_DASHBOARD_URL || 'https://app.foresightaa.co') + '/app/insighter-dashboard/my-downloads'}
