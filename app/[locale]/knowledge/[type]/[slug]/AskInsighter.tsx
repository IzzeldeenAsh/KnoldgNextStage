...
export default function AskInsighter({ knowledgeSlug, questions = [], is_owner = false, onRefreshData }: AskInsighterProps) {
  ...
  const handleQuestionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.foresightaa.co/api/account/ask/insighter/knowledge/${knowledgeSlug}`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({
    ...
    try {
      ...
      if (is_owner) {
        ...
        response = await fetch(
          `https://api.foresightaa.co/api/insighter/library/knowledge/answer/${questionId}`,
          {
            method: 'PUT',
            headers,
            body: JSON.stringify({
  ...
  const deleteQuestion = async (questionId: number) => {
    ...
    try {
      const response = await fetch(
        `https://api.foresightaa.co/api/account/knowledge/question/${questionId}`,
        {
          method: 'DELETE',
          headers: {
            "Content-Type": "application/json",
  ...
  const getLoginUrl = () => {
    return `https://app.foresightaa.co/auth/login?returnUrl=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '')}`;
  };
