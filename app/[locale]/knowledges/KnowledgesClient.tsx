...
async function fetchKnowledgeItems({ taxonomy, id, type, page, locale }: FetchKnowledgeProps): Promise<KnowledgeResponse> {
  ...
  const response = await fetch(`https://api.foresightaa.co/api/platform/industries/type/knowledge?${params.toString()}`, {
    headers: {
...
export default function KnowledgesClient() {
  ...
  useEffect(() => {
    if (selectedId && taxonomy === 'industry') {
      const fetchSubIndustries = async () => {
        try {
          const response = await fetch(`https://api.foresightaa.co/api/platform/industries/${selectedId}/subindustries`, {
            method: 'POST',
            headers: {
              "Content-Type": "application/json",
              "Accept": "application/json",
  ...
  useEffect(() => {
    if (selectedId && taxonomy === 'sub_industry') {
      ...
      fetch(`https://api.foresightaa.co/api/subindustries/${selectedId}/topics`, {
        method: 'POST',
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/json",
