...
export async function fetchAutocomplete(
  ...
  try {
    const response = await fetch(`https://api.foresightaa.co/api/platform/search/autocomplete?keyword=${encodeURIComponent(keyword)}`, {
     headers
    });
...
export async function fetchStatisticsPerType(
  ...
  try {
    const url = new URL('https://api.foresightaa.co/api/platform/search/statistics-per-type');
...
export async function fetchSearchResults(
  ...
  try {
    ...
    const url = new URL('https://api.foresightaa.co/api/platform/search');
